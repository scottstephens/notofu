#!/usr/bin/env python3

import json
import sys

from notofu.signer import get_credentials_via_imdsv2, get_region, get_imdsv2_token, get_proof_req
from notofu.verifier import send_request


def main():
    # print("AWS Instance Credentials Retrieval PoC")
    # print("=" * 40)
    
    # Try IMDSv2 first
    # print("\n1. Attempting to retrieve credentials via IMDSv2...")
    token = get_imdsv2_token()
    imds_region = get_region(token)
    # print(f"region: {imds_region}")

    imds_creds = get_credentials_via_imdsv2(token)
    # print(json.dumps(imds_creds,indent=2))

    host_keys = [
        "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIDDXLx1QDm+4rKdPy1bv6LQYskyTyTu6Tortxxq7+yXq root@nixos",
        "ssh-rsa  root@nixos"
    ]
    req = get_proof_req(imds_creds, 'us-east-2', host_keys)
    req_json = json.dumps(req,indent=2)
    req_dict = json.loads(req_json)
    req_dict['headers']['X-SSH-HOST-KEY-1'] = req_dict['headers']['X-SSH-HOST-KEY-1'] + 'x'
    response = send_request(req_dict)
    
    # print("request:")
    # print(req_json)
    # print()

    print(f"staus_code: {response['status_code']}")
    print(f"content")
    print(response['content'])

if __name__ == "__main__":
    sys.exit(main())
